...
class TapestriSample(object):
    ...
    def barcode_reads(self,
        ...
        info_cmd = 'cutadapt -g %s -O 8 -e 0.2 %s -j 32 --info-file - -o /dev/null --quiet' % (r1_start, r1_in)
        info_file = subprocess.Popen(info_cmd, stdout=subprocess.PIPE, shell=True)

        # generate list of seqs
        if paired_end:
            trim_cmd = 'cutadapt -g %s -G %s -O 8 -e 0.2 %s %s -j 32 -n 3 --interleaved -o - 2> %s' % (r1_start,
                                                                                                       r2_start,
                                                                                                       r1_in,
                                                                                                       r2_in,
                                                                                                       self.cell_cutadapt)
            ...
            trim_cmd = 'cutadapt -g %s -a %s -O 8 -e 0.2 %s -j 32 -n 2 -o - 2> %s' % (r1_start,
                                                                                      r1_end,
                                                                                      r1_in,
                                                                                      self.cell_cutadapt)
